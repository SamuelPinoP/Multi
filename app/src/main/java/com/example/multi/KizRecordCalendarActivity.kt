...
class KizRecordCalendarActivity : SegmentActivity("Weekly Goals View") {
    @Composable
    override fun SegmentContent() {
        KizRecordCalendarScreen()
    }
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun KizRecordCalendarScreen() {
    ...
    LaunchedEffect(Unit) {
        ...
        val storedRec = withContext(Dispatchers.IO) { recDao.getRecords() }
        val storedGoals = withContext(Dispatchers.IO) { goalDao.getGoals() }
        records.clear()
        records.addAll(storedRec.map { it.toModel() })
        goals.clear()
        goals.addAll(storedGoals.map { it.toModel() })
    }

    val completedMap = remember(records, goals) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            HorizontalCalendar(
                ...
                state = state,
                dayContent = { day ->
                    val goalsForDay = completedMap[day.date]
                    val isCurrentMonth = day.position == DayPosition.MonthDate
                    val textColor = when {
                        !goalsForDay.isNullOrEmpty() -> Color(0xFF388E3C)
                    ...
                    val bgColor = when {
                        ...
                        !goalsForDay.isNullOrEmpty() -> Color(0xFFC8E6C9)
                    ...
                    Box(
                        modifier = Modifier
                            ...
                            .clickable(enabled = !goalsForDay.isNullOrEmpty()) {
                                goalsForDay?.let {
                                    selectedGoals = it
                                    showDialog = true
                                }
