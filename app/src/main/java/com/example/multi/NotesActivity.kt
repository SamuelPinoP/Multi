...
private val LocalNotesNavController = staticCompositionLocalOf<NavHostController?> { null }
private val LocalNotesCurrentRoute = staticCompositionLocalOf<String?> { null }
...
class NotesActivity : SegmentActivity("Notes") {
    ...
    override fun handleSegmentBack(): Boolean {
        val controller = notesNavController
        return controller?.popBackStack() == true
    }
    ...
    override fun SegmentContent() {
        ...
        CompositionLocalProvider(
            LocalNotesNavController provides navController,
            LocalNotesCurrentRoute provides currentRoute
        ) {
            NavHost(
                navController = navController,
                startDestination = NotesRoutes.HOME,
                modifier = Modifier.fillMaxSize()
            ) {
                composable(NotesRoutes.HOME) {
                    NotesHomeRoute()
                }
                composable(NotesRoutes.TRASH) {
                    DisposableEffect(Unit) {
                        setSegmentTitle("Trash")
                        importRequest = null
                        onDispose { resetSegmentTitle() }
                    }
                    NotesTrashbinRoute(onNavigateBack = { navController.popBackStack() })
                }
            }
        }
    }
    ...
    override fun OverflowMenuItems(onDismiss: () -> Unit) {
        val navController = LocalNotesNavController.current
        val currentRoute = LocalNotesCurrentRoute.current
        DropdownMenuItem(
            text = { M3Text("Import") },
            enabled = currentRoute != NotesRoutes.TRASH && importRequest != null,
            onClick = {
                onDismiss()
                importRequest?.invoke()
            }
        )
        if (currentRoute == NotesRoutes.TRASH) {
            DropdownMenuItem(
                text = { M3Text("Notes") },
                onClick = {
                    onDismiss()
                    if (navController?.popBackStack() != true) {
                        // Already on start destination, nothing to do
                    }
                }
            )
        } else {
            DropdownMenuItem(
                ...
                onClick = {
                    onDismiss()
                    if (currentRoute != NotesRoutes.TRASH) {
                        navController?.navigate(NotesRoutes.TRASH)
                    }
                }
            )
        }
...
@OptIn(ExperimentalFoundationApi::class)
@Composable
private fun NotesActivity.NotesHomeRoute() {
