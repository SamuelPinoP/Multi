...
private data class GoalEntry(val header: String, val date: String)

class WeeklyGoalsCalendarActivity : SegmentActivity("Weekly Goals Calendar") {
    @Composable
...
private fun WeeklyGoalsCalendarScreen() {
    ...
    val entries = remember { mutableStateListOf<GoalEntry>() }

    LaunchedEffect(Unit) {
        val db = EventDatabase.getInstance(context)
        val goalDao = db.weeklyGoalDao()
        val recordDao = db.weeklyGoalRecordDao()
        val goalEntities = withContext(Dispatchers.IO) { goalDao.getGoals() }
        val recordEntities = withContext(Dispatchers.IO) { recordDao.getRecords() }

        entries.clear()
        val now = LocalDate.now()
        val startCurrent = now.minusDays((now.dayOfWeek.value % 7).toLong())
        goalEntities.map { it.toModel() }.forEach { goal ->
            goal.dayStates.forEachIndexed { index, c ->
                if (c == 'C') {
                    val date = startCurrent.plusDays(index.toLong()).toString()
                    entries.add(GoalEntry(goal.header, date))
                }
            }
        }
        recordEntities.map { it.toModel() }.forEach { rec ->
            val start = LocalDate.parse(rec.weekStart)
            rec.dayStates.forEachIndexed { index, c ->
                if (c == 'C') {
                    val date = start.plusDays(index.toLong()).toString()
                    entries.add(GoalEntry(rec.header, date))
                }
            }
        }
    }
    ...
    var selectedEntries by remember { mutableStateOf<List<GoalEntry>>(emptyList()) }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            HorizontalCalendar(
                ...
                state = state,
                dayContent = { day ->
                    val dayEntries = entries.filter { it.date == day.date.toString() }
                    val isCurrentMonth = day.position == DayPosition.MonthDate
                    val textColor = when {
                        dayEntries.isNotEmpty() -> MaterialTheme.colorScheme.onPrimary
                    ...
                    val bgColor = when {
                        ...
                        dayEntries.isNotEmpty() -> Color(0xFF81C784)
                    ...
                    Box(
                        modifier = Modifier
                            ...
                            .clickable(enabled = dayEntries.isNotEmpty()) {
                                selectedEntries = dayEntries
                                showDialog = true
                            },
