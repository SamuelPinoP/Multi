...
fun EventDialog(
    ...
    var notifyEvent by remember { mutableStateOf(initial.notify) }
        ...
        confirmButton = {
            Button(
                onClick = {
                    ...
                    onSave(title, description, finalDate, address.ifBlank { null }, notifyEvent)
                },
        ...
        text = {
            Column {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    IconButton(onClick = { notifyEvent = !notifyEvent }) {
                        val icon = if (notifyEvent) Icons.Default.Notifications else Icons.Default.NotificationsOff
                        Icon(icon, contentDescription = null, tint = if (notifyEvent) Color.Blue else MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    Text(if (notifyEvent) "On" else "Off", color = if (notifyEvent) Color.Blue else MaterialTheme.colorScheme.onSurfaceVariant)
                }
