...
private fun WeeklyGoalsCalendarScreen() {
    ...
    val goalsByDate = remember { mutableStateMapOf<String, MutableList<WeeklyGoal>>() }
        ...
        goals.forEach { goal ->
            goal.dayStates.forEachIndexed { index, c ->
                if (c == 'C') {
                    ...
                    val list = goalsByDate.getOrPut(date) { mutableListOf() }
                    list.add(goal)
                }
            }
        }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            HorizontalCalendar(
                ...
                state = state,
                dayContent = { day ->
                    val dayGoals = goalsByDate[day.date.toString()] ?: emptyList()
